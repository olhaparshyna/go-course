...
func (s *Slave) declarationAndBinding(_ context.Context, ch *amqp091.Channel) (err error) {
	...
	err = ch.ExchangeDeclare(s.master.exName, "topic", false, false, false, false, nil)
	if err != nil {
		return
	}
...
func (s *Slave) changeChannel(ctx context.Context, channel *amqp091.Channel) {
	s.channel = channel
	s.notifyChanClose = make(chan *amqp091.Error, 1)
	s.notifyConfirm = make(chan amqp091.Confirmation, 1)
