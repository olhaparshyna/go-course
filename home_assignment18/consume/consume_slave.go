...
func (s *Slave) init(ctx context.Context) error {
	...
	// _, err = ch.QueueDeclare(s.master.qName, false, false, false, false, nil)
	// if err != nil {
	// 	return err
	// }
...
func (s *Slave) changeChannel(channel *amqp091.Channel) {
	s.channel = channel
...
func (s *Slave) GetStream() <-chan amqp091.Delivery {
	s.chMux.Lock()
	d := s.delivery
	s.chMux.Unlock()
